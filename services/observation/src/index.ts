...
function log(message: string, meta: any = {}) {
  console.log(JSON.stringify({ ts: new Date().toISOString(), service: "observation", message, ...meta }))
}
...
app.get("/", auth(["observation:read"]), (req: any, res) => {
  log("read", { correlationId: req.correlationId, sub: req.internal?.sub })
  res.json({ service: "observation", ok: true })
})


app.post("/events", auth(["observation:write"]), (_req, res) => {
  res.sendStatus(202)
})


app.listen(7008, () => log("listening", { port: 7008 }))
