...
function log(message: string, meta: any = {}) {
  console.log(JSON.stringify({ ts: new Date().toISOString(), service: "lineage", message, ...meta }))
}
...
app.get("/", auth(["lineage:read"]), (req: any, res) => {
  log("read", { correlationId: req.correlationId, sub: req.internal?.sub })
  res.json({ service: "lineage", ok: true })
})


app.get("/graph", auth(["lineage:read"]), (_req, res) => {
  res.json({ nodes: [], edges: [] })
})


app.listen(7006, () => log("listening", { port: 7006 }))
